...
export default {
  ...
  methods: {
    onLoad() {
      ...
      axios({
        url: `https://m.maizuo.com/gateway?cityId=${this.cityId}&pageNum=${this.current}&pageSize=10&type=1&k=5227612`,
        headers: {
          "X-Client-Info":
  ...
  mounted() {
    http({
      url: `/gateway?cityId=${this.cityId}&pageNum=1&pageSize=10&type=1&k=5227612`,
      headers: {
        "X-Host": "mall.film-ticket.film.list",
      },
    }).then((res) => {
